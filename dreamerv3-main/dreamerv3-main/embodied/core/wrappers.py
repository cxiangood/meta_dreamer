...
class ClipAction(Wrapper):
  ...
  def step(self, action):
    ...
    # Try to coerce clipped value to the dtype declared in the inner env's
    # action space to avoid dtype mismatches (e.g., Python float -> float64).
    try:
      target_space = self.env.act_space[self._key]
      target_dtype = getattr(target_space, 'dtype', None)
    except Exception:
      target_dtype = None
    if target_dtype is not None:
      clipped = np.asarray(clipped, dtype=target_dtype)
    else:
...
class CheckSpaces(Wrapper):
  ...
  def _check(self, value, space, key):
    ...
    # Coerce to numpy array for consistent dtype/shape handling.
    arr = np.asarray(value)
    # Prefer calling the space.contains method when available (more robust),
    # but fall back to membership operator if needed.
    try:
      ...
      # If contains/in operator raised, continue to error reporting below.
      pass
    dtype = arr.dtype
    shape = arr.shape
    lowest, highest = np.min(arr), np.max(arr)
    raise ValueError(
