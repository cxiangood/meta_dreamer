...
def draw_top_down_map_native(
    ...
    if semantic_map:
        ...
        # Slight polygon expansion to close visual gaps between adjacent lanes.
        # Each lane polygon is buffered outward by 0.05m so that neighboring
        # polygons overlap, eliminating anti-aliasing seams from pygame.
        from shapely.geometry import Polygon as ShapelyPolygon
        _LANE_POLYGON_BUFFER = 0.05  # metres

        for obj in all_lanes.values():
            if MetaDriveType.is_lane(obj["type"]) and not draw_center_line:
                poly = obj["polygon"]
                if len(poly) >= 3 and _LANE_POLYGON_BUFFER > 0:
                    sp = ShapelyPolygon(poly).buffer(_LANE_POLYGON_BUFFER)
                    poly = list(sp.exterior.coords)
                pygame.draw.polygon(
                    surface, TopDownSemanticColor.get_color(obj["type"]),
                    [surface.pos2pix(p[0], p[1]) for p in poly]
                )
