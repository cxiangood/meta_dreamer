...
def spline_resample(points, spacing=1.0, smooth_factor=0):
    """Fit a parametric cubic spline through 2D points and resample uniformly.

    Falls back to linear resample_polyline when there are too few points.

    Args:
        points: Nx2 array-like of (x, y) points.
        spacing: target arc-length between output points.
        smooth_factor: smoothing factor passed to splprep (0 = interpolate exactly).

    Returns:
        Nx2 numpy array of resampled points.
    """
    from scipy.interpolate import splprep, splev

    points = np.asarray(points, dtype=float)
    if len(points) < 4:
        return resample_polyline(points, spacing) if len(points) >= 2 else points

    tck, _ = splprep([points[:, 0], points[:, 1]], s=smooth_factor, k=3)

    # Estimate total arc length via fine sampling
    u_fine = np.linspace(0, 1, 1000)
    x_fine, y_fine = splev(u_fine, tck)
    total_length = np.sum(np.sqrt(np.diff(x_fine)**2 + np.diff(y_fine)**2))
    num_output = max(int(total_length / spacing), 10)
