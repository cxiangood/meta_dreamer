...
class BaseMap(BaseRunnable, ABC):
    ...
    def get_semantic_map(
        ...
        # Dilate the road surface region to close sub-pixel gaps between adjacent
        # lane polygons.  Without this, the terrain shader renders grass in the
        # thin seams between neighboring lanes (especially at junction areas).
        road_val = color_setting.get_color(MetaDriveType.LANE_SURFACE_STREET)
        road_mask = (mask[..., 0] == road_val).astype(np.uint8)
        kernel = np.ones((3, 3), np.uint8)
        road_mask = cv2.dilate(road_mask, kernel, iterations=1)
        mask[..., 0] = np.where(road_mask, road_val, mask[..., 0])
