...
class OfflineDataset:
    def __init__(self, data_dir, bev_size=64, seq_len=50,
                 ...
                 return_surrounding_labels=False, n_surrounding=5,
                 merge_endpoints_path=None, merge_zone_radius=25.0):
        ...
        self.return_surrounding_labels = return_surrounding_labels
        self.n_surrounding = n_surrounding
        self.merge_zone_radius = merge_zone_radius

        # Load geometric merge endpoints (per-location fixed points)
        self._merge_endpoints = None
        if merge_endpoints_path and os.path.exists(merge_endpoints_path):
            with open(merge_endpoints_path) as f:
                ep_data = json.load(f)
            self._merge_endpoints = {}
            for loc_str, v in ep_data.items():
                eps = []
                for rg in v.get("ramp_groups", []):
                    for ep in rg.get("merge_endpoints", []):
                        eps.append((float(ep["x"]), float(ep["y"])))
                if eps:
                    self._merge_endpoints[int(loc_str)] = np.array(eps, dtype=np.float32)
            n_eps = sum(len(v) for v in self._merge_endpoints.values())
            print(f"[OfflineDataset] Loaded {n_eps} geometric merge endpoints from {merge_endpoints_path}")
        elif return_phase_labels:
            print("[OfflineDataset] WARNING: return_phase_labels=True but no merge_endpoints_path; "
                  "falling back to per-trajectory merge_frame_idx from npz")
    ...
    def _compute_geometric_merge_frame(self, positions):
        """Find the frame index where vehicle enters the merge zone.

        Uses distance to geometric merge endpoints (fixed per location).
        Returns the first frame where distance < merge_zone_radius.

        Args:
            positions: (T, 2) float32 array of [x, y]

        Returns:
            merge_frame: int, first frame in merge zone, or -1 if not found
        """
        if self._merge_endpoints is None:
            return -1

        T = positions.shape[0]
        # Try all locations — pick the one whose endpoints are closest overall
        best_merge_frame = -1
        best_min_dist = float("inf")

        for loc_id, endpoints in self._merge_endpoints.items():
            # (T, N_eps, 2) -> (T, N_eps)
            dists = np.linalg.norm(
                positions[:, None, :] - endpoints[None, :, :], axis=-1
            )
            min_dists = dists.min(axis=1)  # (T,) min dist to any endpoint of this loc

            # Vehicle must enter and exit the merge zone
            in_zone = min_dists < self.merge_zone_radius
            if in_zone.any():
                first_in = int(np.argmax(in_zone))  # first frame inside radius
                # Check if vehicle eventually passes through (min distance is reasonably small)
                zone_min = min_dists[in_zone].min()
                if zone_min < best_min_dist:
                    best_min_dist = zone_min
                    best_merge_frame = first_in

        return best_merge_frame
    ...
    def _preload_all(self):
        ...
        for fpath in self._file_list:
            try:
                ...
                # Load positions once if needed for phase, speed, or trajectory labels
                need_pos = (self.return_phase_labels or self.return_speed_labels
                ...
                if self.return_phase_labels:
                    if positions is not None and len(positions) > 1:
                        geo_mf = self._compute_geometric_merge_frame(positions)
                        entry["merge_frame_idx"] = geo_mf
                    else:
                        entry["merge_frame_idx"] = int(data.get("merge_frame_idx", -1))
                if self.return_surrounding_labels:
                    surr_path = fpath.replace('.npz', '_surrounding.npz')
                    if os.path.exists(surr_path):
                        surr_data = np.load(surr_path)
                        surr_pos = surr_data['surrounding_positions']  # (T, N, 2)
                        surr_vel = surr_data['surrounding_velocities']  # (T, N, 2)
                        entry["surrounding"] = np.concatenate([surr_pos, surr_vel], axis=-1).astype(np.float32)
    ...
    def sample(self, batch_size):
        ...
        for _ in range(batch_size):
            ...
            if self.return_phase_labels:
                # Use geometric merge frame (distance to fixed endpoints) when available
                positions = data.get("positions", None)
                if positions is not None and len(positions) > 1:
                    geo_mf = self._compute_geometric_merge_frame(positions)
                else:
                    geo_mf = -1
                if geo_mf < 0:
                    # Fallback to per-trajectory merge_frame_idx from npz
                    geo_mf = int(data.get("merge_frame_idx", -1) if "merge_frame_idx" in data else -1)
                frames = np.arange(start, end)
                phase = np.zeros(self.seq_len, dtype=np.int64)
                if geo_mf >= 0:
                    phase[frames >= geo_mf] = 1                         # merge-zone
                    phase[frames >= geo_mf + self.merge_zone_frames] = 2  # post-merge
            ...
            if self.return_surrounding_labels:
                surr_path = self._file_list[fidx].replace('.npz', '_surrounding.npz')
                if os.path.exists(surr_path):
                    surr_data = np.load(surr_path)
                    surr_pos = surr_data['surrounding_positions'][start:end]  # (L, N, 2)
                    surr_vel = surr_data['surrounding_velocities'][start:end]  # (L, N, 2)
                    surr = np.concatenate([surr_pos, surr_vel], axis=-1)  # (L, N, 4)
                else:
                    surr = np.zeros((self.seq_len, self.n_surrounding, 4), dtype=np.float32)
                batch_surrounding.append(torch.from_numpy(surr))

        # Stack
        obs = torch.stack([b[:-1] for b in batch_obs])
        ...
        if self.return_surrounding_labels:
            result += (torch.stack(batch_surrounding),)
        return result
    ...
    def _sample_preloaded(self, batch_size):
        ...
        for _ in range(batch_size):
            ...
            if self.return_surrounding_labels:
                surr = ep.get("surrounding", None)
                if surr is not None:
                    batch_surrounding.append(torch.from_numpy(surr[start:end].copy()))
                else:
                    batch_surrounding.append(torch.zeros(self.seq_len, self.n_surrounding, 4))
    ...
    def sample_start_obs(self, batch_size):
        """Sample starting observations + GT action for imagination training.

        Returns:
            obs: (batch, 3, H, W) float [0,1] — first frame per sample.
            action: (batch, 2) float — GT action at first frame.
        """
        obs, _, actions, _, _ = self.sample(batch_size)
        return obs[:, 0], actions[:, 0]  # (B, 3, H, W), (B, 2)
