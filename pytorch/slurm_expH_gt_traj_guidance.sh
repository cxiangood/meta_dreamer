#!/bin/bash
#SBATCH --job-name=expH_gttraj
#SBATCH --comment="Exp H: GT trajectory guidance — cross-track + heading deviation penalty"
...
#SBATCH --mem=64G
#SBATCH --output=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expH_gttraj_%j.out
#SBATCH --error=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expH_gttraj_%j.err
...
OUTDIR=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expH_gt_traj_guidance
VIDEO_DIR="$OUTDIR/videos_train"
SEL_JSON="$OUTDIR/single_track.json"
mkdir -p "$OUTDIR" "$VIDEO_DIR"

echo "=== Exp H: GT trajectory guidance (cross-track + heading deviation in env reward) ==="
echo "WM: $WM_CKPT (trained, pretrained)"
echo "Key addition: GT human trajectory from CSV used as navigation signal"
echo "  - cross-track error: -2.0 * |lat_dev| (m)"
echo "  - heading deviation: -0.3 * hdg_err (deg)"
echo "  - spatially closest GT point matching (not temporal)"
...
python -u train_online_dreamer_v2.py \
    ...
    --gt-bc-weight 0.3 \
    --wandb-run-name expH_gt_traj_guidance
