#!/bin/bash
#SBATCH --job-name=expF_v2fw
#SBATCH --comment="Exp F v2: frozen WM + residual + phase reward + experience replay"
...
#SBATCH --mem=64G
#SBATCH --output=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expF_v2_%j.out
#SBATCH --error=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expF_v2_%j.err
...
OUTDIR=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expF_v2_frozen_replay
VIDEO_DIR="$OUTDIR/videos_train"
SEL_JSON="$OUTDIR/single_track.json"
mkdir -p "$OUTDIR" "$VIDEO_DIR"

echo "=== Exp F v2: frozen WM + residual + phase reward + 经验回放 ==="
echo "WM: $WM_CKPT (FROZEN — continue/phase heads stay calibrated)"
...
python -u train_online_dreamer_v2.py \
    ...
    --gt-bc-weight 0.3 \
    --wandb-run-name expF_v2_frozen_replay
