#!/bin/bash
#SBATCH --job-name=expI_rewbal
#SBATCH --comment="Exp I: rebalanced reward — halved penalties, +positives, global speed band"
...
#SBATCH --mem=64G
#SBATCH --output=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expI_rewbal_%j.out
#SBATCH --error=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expI_rewbal_%j.err
...
OUTDIR=/share/home/u23516/code/meta_dreamer-main/pytorch/logs/expI_rewbal
VIDEO_DIR="$OUTDIR/videos_train"
SEL_JSON="$OUTDIR/single_track.json"
mkdir -p "$OUTDIR" "$VIDEO_DIR"

echo "=== Exp I: rebalanced reward ==="
echo "Changes:"
echo "  alive bonus:  +0.3 → +0.5"
echo "  nav lateral:  -1.0 → -0.5 × lat_dev"
echo "  global speed: +0.3×(v/30), overspeed penalty, low-speed -0.8"
echo "  Phase 1:      TTC/lateral_overlap/front_gap ALL halved"
echo "  Phase 1:      speed_match +0.3→+0.5, merge_prog +0.2→+0.3"
echo "  Phase 1:      smooth_penalty ×0.5"
echo "  Phase 0:      speed_frac +0.5→+0.2, removed duplicate low-speed"
echo "WM: $WM_CKPT (trained)"
echo "Log: $OUTDIR"
echo "Start: $(date)"
