...
class ClipAction(Wrapper):
  ...
  def step(self, action):
    clipped = np.clip(action[self._key], self._low, self._high).astype(np.float32)
...
#         # 记录原始和目标数据类型
#         self._obs_dtypes = self._get_dtypes(env.obs_space)
#         self._act_dtypes = self._get_dtypes(env.act_space)
    
#     def _get_dtypes(self, space):
#         """获取空间的数据类型映射"""
#         if isinstance(space, gym.spaces.Dict):
#             return {key: self._get_space_dtype(sub_space) for key, sub_space in space.spaces.items()}
#         else:
#             return self._get_space_dtype(space)
    
#     def _get_space_dtype(self, space):
#         """获取单个空间的数据类型"""
...
#             for key, dtype in self._act_dtypes.items():
#                 if key in action:
#                     action[key] = np.asarray(action[key], dtype)
#         else:
#             # 单一动作空间
#             action = np.asarray(action, self._act_dtypes)
        
#         obs = self.env.step(action)
        
#         # 处理观测数据类型转换
#         if isinstance(obs, dict):
#             for key, dtype in self._obs_dtypes.items():
#                 if key in obs:
#                     obs[key] = np.asarray(obs[key], dtype)
#         else:
#             # 单一观测空间
#             obs = np.asarray(obs, self._obs_dtypes)
